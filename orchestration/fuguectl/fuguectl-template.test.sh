...
out="$("$T" impl --set ROLE=backend --set SCOPE=write-parser --set FILES=src/p.py)"
...
out2="$("$T" impl --set ROLE=x)"
ok "unset {{SCOPE}} is kept" 'echo "$out2" | grep -q "{{SCOPE}}"'

# review / analysis templates exist
ok "review template renders" '"$T" review --set REVIEWER=Codex --set DIFF_RANGE=main...HEAD --set DIFF=x | grep -q "VERDICT: ACCEPTED"'
ok "analysis template renders" '"$T" analysis --set ROLE=reviewer | grep -q "must use the Write tool"'

# errors
"$T" >/dev/null 2>&1; ok "no name → non-0" '[ "$?" -ne 0 ]'
"$T" nope >/dev/null 2>&1; ok "unknown template → non-0" '[ "$?" -ne 0 ]'
"$T" impl --set BADFORMAT >/dev/null 2>&1; ok "--set without = → non-0" '[ "$?" -ne 0 ]'
