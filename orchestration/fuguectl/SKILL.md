...
> Tooling: `FO=orchestration/fuguectl/fuguectl` — unified driver: `doctor` `init` `version` `preflight` `smoke` `fleet` `task new|log|done|handoff|digest` `review packet` `template` `dispatch` `cache` `integrate` `allocate` `skills` `workspace` `agents` `experience` `plan` `goal` `loop` `run` `summary` `self-harness` `runtime` `selftest`.
