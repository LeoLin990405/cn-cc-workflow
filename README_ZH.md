...
| `orchestration/fanout/` | `fanout` CLI（18 个子命令）+ `SKILL.md`（5 阶段工作流 + Phase 5 loop）+ `workspaces/` + `templates/` + 18 套测试 |
...
| `fanout selftest` | 跑全部 18 套测试（312 断言） |
...
make test        # cn-plugin + fanout selftest（312 断言）
make doctor      # 环境侦察
make help        # 所有目标
