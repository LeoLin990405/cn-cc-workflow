...
| `orchestration/fanout/` | `fanout` 命令行（14 子命令）+ `SKILL.md`（5 阶段 + Phase 5 loop）+ `workspaces/` + `templates/` + 13 套测试 |
...
`orchestration/fanout/fanout` 是统一入口，`fanout help` 看全表。

| 命令 | 作用 |
|---|---|
| `fanout doctor` | 侦测装了哪些 Agent/CLI + 配了哪些 API → 推荐工作流 |
| `fanout fleet status\|up\|down` | 拉起/查看/停 ccb fleet——剥 `CLAUDE_CODE_*`(防 OAuth 假 401) + detached tmux 起 pane |
| `fanout preflight [cfg]` | go/no-go 门：依赖 · ccbd 存活 · ccb.config 健全 · **no-Gemini 守卫** · `--probe` 活体探端点 |
| `fanout task new\|log\|done` | TASK 文件脚手架 / 日志 / 收尾 |
| `fanout allocate <type> [--top]` | 按 bench 推荐模型（`code`→minimax，`logic`→kimi…） |
...
| `fanout dispatch <target> [--harness ccb\|codex\|opencode] [--workspace ws] [--template n]` | 派活到**任意 harness**（ccb=Claude Code 分身 / codex / opencode）：渲 prompt → 跑 → 记日志 |
...
| `fanout selftest` | 跑全部 13 套测试（119 断言） |
...
3. **Integrate**——各 worktree cherry-pick 到 `main`；跑本地 sanity。
4. **Review**——Codex 给 VERDICT。
5. **Review-Fix Loop**（有界）——确定性门优先 → 增量审 → keep-best（回退退化） → 我 Edit 补丁 → 不收敛元反思；封顶后升级。
...
CI（[`.github/workflows/ci.yml`](.github/workflows/ci.yml)）三 job：**secret-scan**（自定义闸门 + gitleaks）、**shell**（`bash -n` + shellcheck）、**node**（`npm test`）。贡献见 [`CONTRIBUTING.md`](CONTRIBUTING.md)。
...
- **Phase 5 loop** 设计参考 agentic verification loop 文献（Self-Refine、Reflexion、loop-engineering 2026）。
