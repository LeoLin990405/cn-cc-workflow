...
After the selected runtimes pass preflight (`fuguectl doctor`, `fuguectl preflight --harness <name>`, `fuguectl preflight --harness lite` for Codex + OpenCode + Antigravity, and `fuguectl fleet status` when using the `fugue-cc` worktree fleet), use `fuguectl smoke --harness all --codex-clean --out-dir /tmp/fugunano-smoke` when you need a live lite-runtime proof; the output directory includes per-harness transcripts and `summary.json` for machine parsing, and `--task` records the final summary path plus pass/fail counts:
